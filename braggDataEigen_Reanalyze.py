...
def analyze_shot( shot , args, average=False):
  ...
  if ANDOR1 and (args.only == None or 'andor' in args.only):
    results_A= eigenclean_Bragg(atoms_A, noatoms_A, args.discard_A, args.roi_A, args.pixels_A, shotnum, eigenprefix+'_andor', 'andor',Nbgs=args.nbgs, eigenverbose=args.eigenverbose, constbg = args.constbg) 
  ...
  if ANDOR2 and (args.only == None or 'andor2' in args.only):
    results_2 = eigenclean_Bragg(atoms_2, noatoms_2, args.discard_2, args.roi_2, args.pixels_2, shotnum, eigenprefix+'_andor2', 'andor2',Nbgs=args.nbgs, eigenverbose=args.eigenverbose, constbg=args.constbg) 
...
if __name__ == "__main__":
  ...
  parser.add_argument('--eigenverbose', action="store_true",\
          help="Verbose output for eigen analysis")
 
  parser.add_argument('--constbg', action="store_true",\
          help="Include a constant background in basis set")
  ...
  dxA1 = -9
  dyA1 = -8
  args.roi_A = [229+dxA1,227+dyA1,48+22,48+22]
  args.discard_A = [120+dxA1,120+dyA1,272,272]
  args.pixels_A=[]

  # bigger
  for i in range(250+dxA1,284+dxA1):
    for j in range(240+dyA1,276+dyA1):
