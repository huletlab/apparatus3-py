...
def analyze_shot( shot , args, average=False):
  ...
  if MANTA and (args.only == None or 'manta' in args.only):
    results_M = eigenclean_Bragg(atoms_M, noatoms_M, args.discard_M, args.roi_M, args.pixels_M, shotnum, eigenprefix+'_manta', 'manta')
    results_M['section'] = 'MANTAEIGEN' 
  ...
  if ANDOR1 and (args.only == None or 'andor' in args.only):
    results_A= eigenclean_Bragg(atoms_A, noatoms_A, args.discard_A, args.roi_A, args.pixels_A, shotnum, eigenprefix+'_andor', 'andor') 
    results_A['section'] = 'ANDOR1EIGEN' 
  ...
  if ANDOR2 and (args.only == None or 'andor2' in args.only):
    results_2 = eigenclean_Bragg(atoms_2, noatoms_2, args.discard_2, args.roi_2, args.pixels_2, shotnum, eigenprefix+'_andor2', 'andor2') 
    results_2['section'] = 'ANDOR2EIGEN' 
 ...
 #       numpy.savetxt( rangestr + '_ANDOR2.dat', results_2[0]) 
  ...
  if ANDOR1 and ANDOR2:
    for r in results:
      if r['section'] == 'ANDOR1EIGEN':
        andor1signal =  ufloat( ( r['signal'], r['error'] ) )
      if r['section'] == 'ANDOR2EIGEN':
    ...
    report = ConfigObj(inifile)
    report['HHHEIGEN'] = {}
    report['HHHEIGEN']['andor2norm'] = andor2norm.nominal_value
    report['HHHEIGEN']['andor2norm_err'] = andor2norm.std_dev() 
    report.write()
...
if __name__ == "__main__":
  ...
  # MANTA
  args.roi_M = [731,465,18,18]
  args.discard_M = [0,0,1000,1000]
  args.pixels_M=[ (33,30),(33,29), (33,31), (32,29), (32,30), (32,31), (34,29), (34,30), (34,31) ] 

  # ANDOR1
  #args.roi_A = [228,225,48,48]
  args.roi_A = [238,230,48+4,48+4]
  args.discard_A = [0,0,512,512]
  args.pixels_A=[]

  # bigger
  for i in range(244,283):
  ...
  #Data on 130521
  #for i in range(262,278):
  #  for j in range(243,258):
  #    args.pixels_A.append( (i,j) ) 
  ...
  args.roi_2 = [244,255,60,60] 
  args.discard_2 = [8,8,512,512]
  #
  # 2x2
