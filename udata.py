...
datakeys = ['DIMPLELATTICE:knob5', 'SEQ:shot', 'ANDOR1EIGEN:signal' , 'ANDOR2EIGEN:signal', 'HHHEIGEN:andor2norm', 'DIMPLELATTICE:force_lcr3', 'DIMPLELATTICE:tof', 'DIMPLELATTICE:imgdet' ]
def K( key ) : 
...
def plotkey( ax, gk, key, dat, base, marker='o', ms=5.,\
    ...
    if raw:
        ax.plot( fx(dat[:, K('DIMPLELATTICE:knob5')])+raw_offset, dat[:,K(key)], '.',
                 marker='o', mec=mec, ms=3, mew=1.0, 
                 color='gray', alpha=0.5) 
     
    datS = statdat.statdat( dat, K('DIMPLELATTICE:knob5'), K(key) )
    alldat.append(datS) 
...
for k in sorted(gdat.keys()):
    ...
    print
    print "KEY = ",k
    print "TOFDATA @", np.unique(tofdat[:,K('DIMPLELATTICE:knob5')])
    ...
    tofset = set( np.unique(tofdat[:,K('DIMPLELATTICE:knob5')]).tolist() )
    datset = set( np.unique(dat[:,K('DIMPLELATTICE:knob5')]).tolist() )
    common = list( tofset & datset )

    np.set_printoptions(suppress=True, precision=3)
    for i,c in enumerate(sorted(common)):
        tofi = tofdat[ tofdat[:,K('DIMPLELATTICE:knob5')] == c ]
        dati = dat[ dat[:,K('DIMPLELATTICE:knob5')] == c ]
        print '\nKNOB5 = ', c 
    
        #print 
        #knob5 = 200. 
        cols = ( K('SEQ:shot'), K('ANDOR1EIGEN:signal'), K('ANDOR2EIGEN:signal'), \
                 K('HHHEIGEN:andor2norm'), K('DIMPLELATTICE:tof') ) 
        print "DAT"
        print dati[:,cols]
        print "TOFDAT"
        print tofi[:,cols]
