...
ANDOR1TOFEIGEN_sec = 'ANDOR1EIGEN_TOF'
ANDOR2EIGEN_sec = 'ANDOR2EIGEN'
ANDOR2TOFEIGEN_sec = 'ANDOR2EIGEN_TOF'
...
def analyze_shot( shot , args, average=False):
  ...
  if not args.ignoreA1:
      try : 
        if average:
          shotnum = args.shots[0]
          atoms     = pyfits.open( shotnum + 'atoms_tof.fits')[0].data[0]
          atoms_A_tof = numpy.zeros_like( atoms)
          noatoms_A_tof = numpy.zeros_like( atoms)
          nshots = 0
          binning_A_tof =  (512/atoms.shape[0], 512/atoms.shape[1]) 
          
          for shotnum in args.shots: 
            try:
              atoms     = pyfits.open( shotnum + 'atoms_tof.fits')[0].data[0]
              noatoms   = pyfits.open( shotnum + 'noatoms_tof.fits')[0].data[0]
              atoms_A_tof = atoms_A + atoms
              noatoms_A_tof = noatoms_A + noatoms
              nshots = nshots + 1
            except:
              print "Error obtaining data from shot #%d" % shotnum
        
          atoms_A_tof = atoms_A_tof / nshots
          noatoms_A_tof = noatoms_A_tof / nshots 
        else:
          atoms     = pyfits.open( shotnum + 'atoms_tof.fits')[0].data[0]
          noatoms   = pyfits.open( shotnum + 'noatoms_tof.fits')[0].data[0]
          atoms_A_tof = atoms
          noatoms_A_tof = noatoms
          binning_A_tof =  (512/atoms.shape[0], 512/atoms.shape[1])
        ANDOR1_tof = True
      except Exception as e:
        print e
  ...
  try : 
    if average:
      shotnum = args.shots[0]
      atoms     = pyfits.open( shotnum + 'atoms_tof_andor2.fits')[0].data[0]
      atoms_2_tof = numpy.zeros_like( atoms)
      noatoms_2_tof = numpy.zeros_like( atoms)
      nshots = 0
      binning_2 =  (512/atoms.shape[0], 512/atoms.shape[1]) 
      ...
      for shotnum in args.shots: 
        try:
          atoms     = pyfits.open( shotnum + 'atoms_tof_andor2.fits')[0].data[0]
          noatoms   = pyfits.open( shotnum + 'noatoms_tof_andor2.fits')[0].data[0]
          atoms_2_tof = atoms_2 + atoms
          noatoms_2_tof = noatoms_2 + noatoms
          nshots = nshots + 1
        except:
          print "Error obtaining data from shot #%d" % shotnum
    
      atoms_2_tof = atoms_2 / nshots
      noatoms_2_tof = noatoms_2 / nshots 
    else:
      atoms     = pyfits.open( shotnum + 'atoms_tof_andor2.fits')[0].data[0]
      noatoms   = pyfits.open( shotnum + 'noatoms_tof_andor2.fits')[0].data[0]
      atoms_2_tof = atoms
      noatoms_2_tof = noatoms
      binning_2_tof =  (512/atoms.shape[0], 512/atoms.shape[1]) 
    print atoms.shape 
    ANDOR2_tof = True
  except Exception as e:
    print e
  ...
  if ANDOR1_tof and (args.only == None or 'andor' in args.only):
    results_A_tof= eigenclean_Bragg(atoms_A_tof, noatoms_A_tof, args.discard_A, args.roi_A, args.pixels_A, shotnum, eigenprefix+'_andor_tof', 'andor',Nbgs=args.nbgs) 
    results_A_tof['section'] = ANDOR1TOFEIGEN_sec 
    results.append(results_A_tof)
    print 'ANDOR1_TOF\t',
    print bcolors.WARNING + "%7.1f +/- %6.1f "%(results[-1]['signal'],results[-1]['error']) + bcolors.ENDC,
    print "mean_out=%.2f" % results[-1]['mean_out'],
    print "stdev_out=%.2f" % results[-1]['stdev_out']
  elif not ANDOR1:
    print 'ANDOR1_TOF\t  no pictures'
  else:
    print 'ANDOR1_TOF\t  skipping'
  ...
  if ANDOR2 and (args.only == None or 'andor2' in args.only):
    results_2_tof = eigenclean_Bragg(atoms_2_tof, noatoms_2_tof, args.discard_2, args.roi_2, args.pixels_2, shotnum, eigenprefix+'_andor2_tof', 'andor2',Nbgs=args.nbgs) 
    results_2_tof['section'] = ANDOR2TOFEIGEN_sec 
    results.append(results_2_tof)
    print 'ANDOR2_TOF\t',
    print bcolors.WARNING + "%7.1f +/- %6.1f "%(results[-1]['signal'],results[-1]['error']) + bcolors.ENDC,
    print "mean_out=%.2f" % results[-1]['mean_out'],
    print "stdev_out=%.2f" % results[-1]['stdev_out']
  elif not ANDOR2:
    print 'ANDOR2_TOF\t  no pictures'
  else:
    print 'ANDOR2_TOF\t  skipping'
...
if __name__ == "__main__":
  ...
  #args.pixels_A=[]
  args.roi_A = [25,29,12,12] 
  args.discard_A = [8,8,48,48]
