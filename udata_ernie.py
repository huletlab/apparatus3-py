...
parser.add_argument('--binn', action="store",type=float, default=500,\
...
#if args.key.split(":")[0] in extraKeys:
#	datakeys2 = ['DIMPLE:ir1pow0', 'SEQ:shot', 'ANDOR1EIGEN:signal',\
#            'ANDOR2EIGEN:signal', 'HHHEIGEN:andor2norm',\
#            'DIMPLELATTICE:force_lcr3', 'DIMPLELATTICE:tof',\
#            'DIMPLELATTICE:imgdet', 'DIMPLELATTICE:image' ]
#	gdat2 = {} 
...
gs = matplotlib.gridspec.GridSpec( 2,5, wspace=0.4, hspace=0.24,\
...
axS1_new = plt.subplot( gs[0,4] ) 
axS2_new = plt.subplot( gs[1,4] ) 
...
for k in sorted(gdat.keys()):
    dat = gdat[k]['data']
    gdat[k]['data']=np.insert(dat,len(dat[0]),0,axis=1)
    print gdat[k]['data']
    tofLock_cond = [ ('DIMPLELATTICE:tof', args.tofval),\
                     ('DIMPLELATTICE:imgdet', args.braggdet)]
    if not args.varyimage:
	...
	for n,i in enumerate(inSitui):
		...
		for l in gdat[k]['data']:
			if l[K("SEQ:shot")]==i[K('SEQ:shot')]:
				l[-1]=roundUpToMultiple(tofLocki[index,K('ANDOR1EIGEN:signal')],args.binn)/1000
				break
		for l in gdat[k]['data']:
    ...
    plotkey_ratio( axS1_new, gdat[k], newK, fx, "A1tofCount", 'ANDOR1EIGEN:signal',\
                   inSitu_cond, tofLock_cond, gdat[k]['data'], 1.0, save=False, labelstr='TOF', yf=SQ1)
    plotkey_ratio( axS2_new, gdat[k], newK, fx, "A1tofCount", 'ANDOR2EIGEN:signal',\
                   inSitu_cond, tofLock_cond, gdat[k]['data'], 1.0, save=False, labelstr='TOF', yf=SQ2)
...
axS1_new.set_ylabel(r'$S1$', ha='center', labelpad=20, rotation=0, fontsize=18)
axS2_new.set_ylabel(r'$S2$', ha='center', labelpad=20, rotation=0, fontsize=18)


axes = [ ax1, axA1, axA2, ax1T, axA1T, axA2T, axS1, axS2,axS1_new,axS2_new]
for ax in axes:
    ax.grid()
    if args.xkey != aSkey:
