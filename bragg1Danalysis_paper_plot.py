...
def process(diff):
    ...
    a_guess = (y.max() - y.min())*0.3
    mu1_guess, mu2_guess = float(y[y.argmax():y.argmax()+10].argmin()+y.argmax()), float(y[:len(y)/3*2].argmax())
    sigma_guess = np.abs(mu2_guess - mu1_guess)/20 
    ...
    param_bounds=([0,mu1_guess-10,mu2_guess-10,0,-10],[a_guess*3,mu1_guess+10,mu2_guess+3,2.5,10]	)
...
#
#			try:
#	            		return bragg_1D_anlysis(datadir, cddata_bootstrap, np.copy(cddata_ref_dic_delta[delta]), report=report,
...
#		pool.close()
#    		pool.join()
#		excitation_boot = [ r[0] for r in results if r]
#		contrast_boot = [ r[1] for r in results if r]
#		print "ok Here"
		tempFlag = kwargs['save_fig'] 
		kwargs['save_fig'] = False
		if delta in cddata_mean_dict_bootstrap:
		...
		kwargs['save_fig'] = tempFlag
			...
			try:
				...
				excitation_bootstrap.append([0,0])
				contrast_bootstrap.append([0,0])	
				print "Fit Fail"
...
#    print len(deltas)
    ...
    print len(deltas),len(excitation_bootstrap[:,0]) 
    try:
...
def bragg_multi(datadir, shots, key=None,no_bragg_ref=0, **kwargs):
	...
	for group_key in report_group_dict.keys():
	    ...
	    if density_group_dict[group_key]:
            	oneDsample_lib.process([max(density_group_dict[group_key])], fname='1Dsample_%s_%s.png'%(key[1],group_key),folder = os.path.join(datadir,'1D_analysis'),save_txt=True)
            	oneDsample_lib.process(density_group_dict[group_key], fname='1Dsample_%s_%s_ave.png'%(key[1],group_key),folder = os.path.join(datadir,'1D_analysis'),save_txt=True)
