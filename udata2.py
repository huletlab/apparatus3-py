...
if args.xkey != aSkey:
    ...
    xkeystr = '' 
outfile = savepath + "udata2_" + datestamp + output + xkeystr + ".png" 
print outfile
...
for k in sorted(gdat.keys()):
    ...
    k05 =  np.unique( alldat[:,K('DIMPLELATTICE:knob05')] )
    color = ['blue', 'red', 'green'] 
    for cc,aS in enumerate(k05):

        print 
        print "### KNOB05 = %d"% aS
   
        dat = alldat[ alldat[:,K('DIMPLELATTICE:knob05')] == aS ]  
        ...
        tofLock_label =  '$10\,\mu\mathrm{s}$ TOF, as=%d'%aS
        tofLock_offset = -0.4
        tofLock_color = color[cc] 
        ...
        insitu_label = 'In-situ, as=%d'%aS
        insitu_offset = 0.4
        insitu_color = color[cc]
        plotkey( ax1, gdat[k], K, fx, args.xkey, 'HHHEIGEN:andor2norm',\
                 inSitu, base1, mec=insitu_color, mfc=insitu_color, labelstr=insitu_label,save=False,
                 raw=True, raw_offset=insitu_offset) 
        plotkey( axA1, gdat[k], K, fx, args.xkey, 'ANDOR1EIGEN:signal',\
                  inSitu, 1., mec=insitu_color, mfc=insitu_color, labelstr=insitu_label,save=False,
                 raw=True, raw_offset=insitu_offset) 
        ...
        plotkey_ratio( ax1T, gdat[k], K, fx, args.xkey, 'HHHEIGEN:andor2norm',\
                       inSitu_cond, tofLock_cond, dat, 1.0, labelstr=label, \
                       mec=insitu_color, mfc=insitu_color, exceptions=True)
        plotkey_ratio( axA1T, gdat[k], K, fx, args.xkey, 'ANDOR1EIGEN:signal',\
                       inSitu_cond, tofLock_cond, dat, 1.0, save=False, labelstr=label,\
                       mec=insitu_color, mfc=insitu_color, )
        ...
        plotkey_ratio( axS1, gdat[k], K, fx, args.xkey, 'ANDOR1EIGEN:signal',\
                       inSitu_cond, tofLock_cond, dat, 1.0, \
                       mec=insitu_color, mfc=insitu_color,\
                       save=False, labelstr=label, yf=SQ1)
        plotkey_ratio( axS2, gdat[k], K, fx, args.xkey, 'ANDOR2EIGEN:signal',\
                       inSitu_cond, tofLock_cond, dat, 1.0, \
                       mec=insitu_color, mfc=insitu_color,\
                       save=False, labelstr=label, yf=SQ2)
